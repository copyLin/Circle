...
public class LostDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        comments = (RecyclerView) findViewById(R.id.lost_comments);
        ...
        final LostItem item = (LostItem) intent.getSerializableExtra("item");
        userName.setText(item.getUserName());
        sendTime.setText(item.getSendTime());
        content.setText(item.getContent());
        lostTime.setText(item.getEventTime());
        lostContact.setText(item.getContact());
        Glide.with(this).load(R.string.server_ip + "user_img/" + item.getUserImg()).into(mCircleImageView);

        List<String> imgPaths = new ArrayList<>();
        for (String imgPath : item.getLostImgs()){
